...
class Missions extends PureComponent {
    ...
    yearClickHandler=(e,year)=>{
        if(year!==undefined && year!==this.state.year){
            ...
            const landFilterValue=this.state.lanchFilterValue;
    ...
    render() {
        ...
        const missionsData = data.map(el=>(
            <Mission
            key={el.mission_name}
        ...
        const rows=years.map((el,i,arr)=>{
            let res=null;
            if(i%2===0){
            res=<tr key={arr[i]} onClick={this.yearClickHandler}>
                <TableColumn
        ...
        const table= <table cellSpacing="22" cellPadding="10">
            
            <thead><tr><th colSpan="2" style={{textDecoration:'underline'}}>Launch Year</th></tr></thead>
        <tbody> {rows}
        <tr>
            <td colSpan="2" style={{background:'white'}}><b style={{textDecoration:'underline'}}>Launch Successful</b></td>
        </tr>
        <tr>
            <td
            style={{background:this.state.appLiedLanchFilterColor.true}}
            onClick={(event)=>this.isLaunchSuccessfulHandler(event,"true")}>True</td>
            <td 
            style={{background:this.state.appLiedLanchFilterColor.false}}
            onClick={(event)=>this.isLaunchSuccessfulHandler(event,"false")}>False</td>
        </tr>

        <tr>
            <td colSpan="2" style={{background:'white'}}><b style={{textDecoration:'underline'}}>Land Successful</b></td>
        </tr>
        <tr>
            <td
            style={{background:this.state.appliedLandFilterColor.true}}
            onClick={(event)=>this.isLandSuccessfulHandler(event,"true")}>True</td>
            <td 
            style={{background:this.state.appliedLandFilterColor.false}}
            onClick={(event)=>this.isLandSuccessfulHandler(event,"false")}>False</td>
        </tr>
        </tbody>
