...
describe("<Mission />", () => {
  ...
  it("should have 0 Loader when loading state is false", () => {
  ...
  it("should have <footer>Developed by: Santosh</footer>", () => {
    expect(component.contains(<footer>Developed by: Santosh</footer>)).toEqual(
      true
    );
  });

  //  it('should contain text "There is no Data available for aplied filter" </footer>',()=>{
  //     component.setState({missions:[]})
  //  expect(component.find('.Missions').text()).toEqual('There is no Data available for aplied filter');
  //  })
});
